...
const Loanstypes = () => {
    ...
    const fetchExistingLoanTypes = useCallback(async () => {
        ...
        try {
            ...
            const response = await fetch(`${API_BASE_URL}/loanTypes/all`, {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (editingLoanId) {
                // PUT request to update (admin-only)
                ...
                // POST request to add (admin-only)
    ...
    return (
        <div className="container-fluid mt-5">
            <div className="d-flex justify-content-between align-items-center mb-4 mt-4">
                <h4 className="mb-0">Loan Types Management</h4>
                <Button onClick={() => {
                    setEditingLoanId(null);
                    setLoanData(initialLoanState);
                    setShowForm(true);
            ...
            {showForm && (
                <Card className="mb-4 bg-white border-0">
                    <Card.Header as="h5" className='bg-white border-0'>{editingLoanId ? 'Edit Loan Type' : 'Add New Loan Type'}</Card.Header>
                    <Card.Body>
                        ...
                        <Form onSubmit={handleSubmit}>
                                ...
                                <Row key={index} className="mb-2 align-items-center">
                                    ...
                                    <Col md={1} className="text-end">
                                        {loanData.descriptionPoints.length > 1 && (
                                            <Button variant="outline-danger" size="sm" onClick={() => removeDescriptionPoint(index)}>
                                                X
                                            </Button>
                                        )}
                                    </Col>
                            ...
                            <Button variant="outline-secondary" size="sm" onClick={addDescriptionPoint} className=' border-0 totobutton'>Add Description Point</Button>
                ...
                {!isLoadingLoanTypes && existingLoanTypes.length > 0 && (
                    ...
                    <ListGroup>
                        {existingLoanTypes.map((loan) => (
                            <ListGroup.Item
                                key={loan.id}
                                className="d-flex justify-content-between align-items-center flex-wrap border-0 shadow-sm"
                            >
